...
export default function I3LaunchpadDetail({ agency }: { agency: Agency }) {
  ...
  const galleryImages = ['/photo/S43A9142.jpg', '/photo/S43A9196.jpg', '/photo/S43A0049.jpg'];
  const offeringImages = ['/photo/S43A0049.jpg', '/photo/S43A9091.jpg', '/images/_DSC2948.jpg', '/photo/S43A0059.jpg'];
  ...
  return (
    <main className='min-h-screen bg-white'>
      ...
      {/* ── Overview — vertical milestone path ── */}
      <section className='py-20 md:py-28 bg-white overflow-hidden'>
        <div className='max-w-7xl mx-auto px-6 md:px-12'>
          <div ref={overviewSection.ref} className='grid grid-cols-1 lg:grid-cols-2 gap-14 items-center'>
            ...
            <div className='order-1 lg:order-2 flex flex-col gap-6' style={fadeAnim(overviewSection.inView, 0.15)}>
              {galleryImages.map((img, i) => (
                <div key={i} className='flex items-center gap-4'>
                  <div className='flex flex-col items-center flex-shrink-0 self-stretch'>
                    <div className='w-8 h-8 rounded-full flex items-center justify-center shadow-md flex-shrink-0' style={{ backgroundColor: ACCENT }}>
                      <span className='text-white text-[11px] font-bold' style={{ fontFamily: 'League Spartan, sans-serif' }}>{i + 1}</span>
                    </div>
                    {i < galleryImages.length - 1 && <div className='w-px flex-1 mt-1' style={{ backgroundColor: 'rgba(48,108,236,0.25)', minHeight: '24px' }} />}
                  </div>
                  <div className='rounded-2xl overflow-hidden shadow-xl relative flex-1'>
                    <img src={img} alt={agency.name} className='w-full h-40 object-cover hover:scale-105 transition-transform duration-700' />
        ...
        <div className='max-w-7xl mx-auto relative'>
          <div ref={offeringsSection.ref}>
            <div className='text-center max-w-xl mx-auto mb-14 px-6' style={anim(offeringsSection.inView, 0)}>
              <h2 className='font-bold text-gray-900' style={{ fontFamily: 'League Spartan, sans-serif', fontSize: 'clamp(1.8rem, 3.5vw, 2.6rem)' }}>Every Stage, Covered</h2>
            </div>
      ...
      {/* ── The Growth Path — ascending process ── */}
      <section className='py-20 md:py-28' style={{ backgroundColor: '#fafafa' }}>
        <div className='max-w-7xl mx-auto px-6 md:px-12'>
          <div ref={approachSection.ref}>
            <div className='flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-16'>
              <div>
                <h2 className='font-bold text-gray-900' style={{ fontFamily: 'League Spartan, sans-serif', fontSize: 'clamp(1.8rem, 3.5vw, 2.6rem)', ...anim(approachSection.inView, 0) }}>The Growth Path</h2>
              </div>
            ...
            <div className='hidden md:block relative'>
              ...
              <div className='grid grid-cols-5 gap-4'>
                {agency.steps.map((step, idx) => (
                  ...
                  >
                    {/* Permanent ascending offset lives on this inner layer — a CSS animation on the outer
                        element would otherwise override the outer's own transform once slideInUp completes. */}
                    <div style={{ transform: `translateY(-${idx * 16}px)` }}>
      ...
      <section className='relative py-28 md:py-36 overflow-hidden' style={{ backgroundImage: `url(${agency.overviewImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className='max-w-3xl mx-auto px-6 md:px-12 text-center relative'>
          <div ref={ctaSection.ref}>
            <div className='flex justify-center mb-4' style={anim(ctaSection.inView, 0)}>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#7ea6f5" className="w-5 h-5">
